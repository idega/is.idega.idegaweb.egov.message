...
public class DocumentBusinessBean extends com.idega.business.IBOServiceBean implements DocumentBusiness {
	...
	/*
	 * Commented out since it is never used... private Font getDefaultTagFont() { if (defaultTagFont == null) { defaultTagFont = new
	 * Font(Font.HELVETICA); defaultTagFont.setSize(12); } return defaultTagFont; }
	 */
	...
	/*
	 * Commented out since it is never used... private Font getDefaultTitleFont() { if (defaultTitleFont == null) { defaultTitleFont = new
	 * Font(Font.HELVETICA); defaultTitleFont.setSize(12); } return defaultTitleFont; }
	 */
	...
	/*
	 * Commented out since it is never used... private Font getTagFont(PrintedLetterMessage msg) { return getDefaultTagFont(); }
	 */

	/*
	 * Commented out since it is never used... private Font getParagraphFont(PrintedLetterMessage msg) { return getDefaultParagraphFont(); }
	 */
	 ...
	 * @throws NoUserAddressException
	 */
	private Address getPostalAddress(User user) throws RemoteException {
		Address address = getUserBusiness().getUserAddressByAddressType(((Integer) user.getPrimaryKey()).intValue(), getUserBusiness().getAddressHome().getAddressType2());
		if (address == null || address.getStreetAddress().length() == 0) {
	...
	private int createContent(DocumentPrintContext dpc) throws ContentCreationException {
		...
		if (msg instanceof PrintedLetterMessage) {
			...
			if (pmsg.getLetterType().equals(MessageConstants.LETTER_TYPE_PASSWORD)) {
				...
				return createDefaultLetterContent(dpc);
			}
		}
		return 0;
	...
	private void handleNoAddressUser() {
		/*
		 * try{ String status = getMessageBusiness().getCaseStatusError().getStatus(); getMessageBusiness().flagMessageWithStatus(performer,msg,status); }
	...
	/*
	 * Commented out since it is never used... private class NoLetterTemplateException extends Exception{ NoLetterTemplateException(String type){
	 * super("Letter template for case type "+type +" not found."); } }
	 */
	/*
	 * (non-Javadoc)
	 * 
