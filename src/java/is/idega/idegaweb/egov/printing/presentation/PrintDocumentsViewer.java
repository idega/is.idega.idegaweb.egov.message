...
import is.idega.idegaweb.egov.printing.business.DocumentBusiness;
import is.idega.idegaweb.egov.printing.business.DocumentService;
import is.idega.idegaweb.egov.printing.data.PrintDocuments;

/**
 * Title: PrintedLetterViewer Description: A class to view and manage PrintedLetterMessage in the idegaWeb Commune application Copyright: Copyright
 * idega Software (c) 2002 Company: idega Software
 *
...
public class PrintDocumentsViewer extends CommuneBlock {
	...
	public void init(IWContext iwc) throws RemoteException {
		...
		this.statusUnprinted = msgBuiz.getCaseStatusOpen().getStatus();
		this.statusPrinted = msgBuiz.getCaseStatusReady().getStatus();
		this.statusDeleted = msgBuiz.getCaseStatusInactive().getStatus();
	...
	public void add(PresentationObject po) {
		if (this.mainTable == null) {
			...
			this.mainTable.setColor(getBackgroundColor());
			this.mainTable.setWidth(getWidth());

		}
		this.mainTable.add(po);
	}
	...
	private int parseAction(IWContext iwc) throws Exception {
		...
		if (iwc.isParameterSet(PARAM_LETTER_TYPE)) {
			this.currentType = iwc.getParameter(PARAM_LETTER_TYPE);
		}
		if (iwc.isParameterSet(PARAM_LETTER_STATUS)) {
			this.currentStatus = iwc.getParameter(PARAM_LETTER_STATUS);
		}
		/** @TODO: get this status from beans */
		if ("".equals(this.currentStatus)) {
			this.currentStatus = (this.statusUnprinted);
		}
		...
		if (iwc.isParameterSet(PRM_BULK_VIEW)) {
			this.isBulkManual = true;
		}

		this.isBulkType = getDocumentBusiness(iwc).isBulkLetterType(this.currentType);

		if (iwc.isParameterSet(PARAM_MESSAGE_ID)) {
			this.msgID = Integer.parseInt(iwc.getParameter(PARAM_MESSAGE_ID));
		}
		return action;
	}

	private void initDates(IWContext iwc) throws Exception {
		this.pFrom = IWTimestamp.RightNow();
		this.pFrom.addDays(-this.defaultDays);
		this.pTo = IWTimestamp.RightNow();
		this.uFrom = IWTimestamp.RightNow();
		this.uFrom.addDays(-this.defaultDays);
		this.uTo = IWTimestamp.RightNow();
		if (iwc.isParameterSet(PRM_STAMP_P_FROM)) {
			try {
				long time = Long.parseLong(iwc.getParameter(PRM_STAMP_P_FROM));
				this.pFrom = new IWTimestamp(time);
			}
			catch (NumberFormatException e) {
				this.pFrom = new IWTimestamp(iwc.getParameter(PRM_STAMP_P_FROM));
			}
			this.pFrom.setTime(0, 0, 0);
		}
		if (iwc.isParameterSet(PRM_STAMP_U_FROM)) {
			try {
				long time = Long.parseLong(iwc.getParameter(PRM_STAMP_U_FROM));
				this.uFrom = new IWTimestamp(time);
			}
			catch (NumberFormatException e) {
				this.uFrom = new IWTimestamp(iwc.getParameter(PRM_STAMP_U_FROM));
			}
			this.uFrom.setTime(0, 0, 0);
		}
		if (iwc.isParameterSet(PRM_STAMP_P_TO)) {
			try {
				long time = Long.parseLong(iwc.getParameter(PRM_STAMP_P_TO));
				this.pTo = new IWTimestamp(time);
			}
			catch (NumberFormatException e) {
				this.pTo = new IWTimestamp(iwc.getParameter(PRM_STAMP_P_TO));
			}
			this.pTo.setTime(23, 59, 59);
		}
		if (iwc.isParameterSet(PRM_STAMP_U_TO)) {
			try {
				long time = Long.parseLong(iwc.getParameter(PRM_STAMP_U_TO));
				this.uTo = new IWTimestamp(time);
			}
			catch (NumberFormatException e) {
				this.uTo = new IWTimestamp(iwc.getParameter(PRM_STAMP_U_TO));
			}
			this.uTo.setTime(23, 59, 59);
		}
	...
	private void initCursors(IWContext iwc) throws Exception {
		if (iwc.isParameterSet(PRM_CURSOR_P)) {
			this.cursor_p = Integer.parseInt(iwc.getParameter(PRM_CURSOR_P));
		}
		if (iwc.isParameterSet(PRM_CURSOR_U)) {
			this.cursor_u = Integer.parseInt(iwc.getParameter(PRM_CURSOR_U));
		}
		if (iwc.isParameterSet(PRM_P_COUNT)) {
			this.count_p = Integer.parseInt(iwc.getParameter(PRM_P_COUNT));
		}
		if (iwc.isParameterSet(PRM_U_COUNT)) {
			this.count_u = Integer.parseInt(iwc.getParameter(PRM_U_COUNT));
		}
	}
	...
	private void printAllUnPrintedMessages(IWContext iwc) throws Exception {
		...
		Collection<PrintMessage> unPrintedLetters = getDocumentBusiness(iwc).getUnPrintedMessages(this.currentType, -1, -1);
		/*
		 * getDocumentBusiness(iwc).writeBulkPDF( unPrintedLetters, iwc.getCurrentUser(), localize("printdoc.bulkletter_filename","BulkLetterPDF"),
		 * iwc.getApplicationSettings().getDefaultLocale(), currentType, false, true, true);
		 */
		getDocumentService(iwc).createPDF(iwc, unPrintedLetters, this.currentType, StringHandler.stripNonRomanCharacters(localize("printdoc.bulkletter_filename", "BulkLetterPDF")), true);
	}

	private void printMessage(IWContext iwc) throws Exception {
		// int userID = ((Integer) iwc.getCurrentUser().getPrimaryKey()).intValue();
		if (this.msgID > 0) {
			PrintedLetterMessage msg = (PrintedLetterMessage) getDocumentBusiness(iwc).getPrintedLetterMessageHome().findByPrimaryKey(new Integer(this.msgID));
			// fileID
			// =getDocumentBusiness(iwc).writePDF(msg,iwc.getCurrentUser(),localize("printdoc.letter_filename","LetterPDF"),iwc.getApplicationSettings().getDefaultLocale(),true);

			// TODO
			this.fileID = getDocumentService(iwc).createPDF(iwc, msg, StringHandler.stripNonRomanCharacters(localize("printdoc.letter_filename", "LetterPDF")), false).intValue();
	...
	private void printSelected(IWContext iwc) throws Exception {
		...
		this.isBulkManual = bulk;
		boolean flag = !iwc.isParameterSet("prv_mark");
		String[] ids = iwc.getParameterValues(PRM_U_CHK);
		if (ids != null && ids.length > 0) {
			/*
			 * getDocumentBusiness(iwc).writeBulkPDF(ids, iwc.getCurrentUser(), localize("printdoc.bulkletter_filename","BulkLetterPDF"),
			 * iwc.getApplicationSettings().getDefaultLocale(), currentType, true, flag, bulk);
			 */
			 ...
			 *
			 * for (int i = 0; i < ids.length; i++) { PrintedLetterMessage msg =phome.findByPrimaryKey(new Integer(ids[i]));
			 * serv.createPDF(iwc,msg,"letter.pdf",false); }
			 */
			serv.createPDF(iwc, ids, this.currentType, "letters.pdf", flag);
		}
	}
	...
	private void reprintSelected(IWContext iwc) throws Exception {
		...
		if (ids != null && ids.length > 0) {
			/*
			 * getDocumentBusiness(iwc).writeBulkPDF( ids, iwc.getCurrentUser(), localize("printdoc.bulkletter_filename","BulkLetterPDF"),
			 * iwc.getApplicationSettings().getDefaultLocale(), currentType, true, false, true);
			 */
			DocumentService serv = getDocumentService(iwc);
			/*
			 * PrintedLetterMessageHome phome = getDocumentBusiness(iwc).getPrintedLetterMessageHome(); for (int i = 0; i < ids.length; i++) {
			 * PrintedLetterMessage msg =phome.findByPrimaryKey(new Integer(ids[i])); serv.createPDF(iwc,msg,"letter.pdf",false); }
			 */
			serv.createPDF(iwc, ids, this.currentType, "letters.pdf", false);
		}
	}
	...
	private void deleteSelected(IWContext iwc) throws Exception {
		String[] ids = iwc.getParameterValues(PRM_U_CHK);
		if (ids != null && ids.length > 0) {
			getMessageBusiness(iwc).flagMessagesWithStatus(iwc.getCurrentUser(), ids, this.statusDeleted);
		}
	}
	...
	private void undeleteSelected(IWContext iwc) throws Exception {
		String[] ids = iwc.getParameterValues(PRM_U_CHK);
		if (ids != null && ids.length > 0) {
			getMessageBusiness(iwc).flagMessagesWithStatus(iwc.getCurrentUser(), ids, this.statusUnprinted);
		}
	}
	...
	private void addTypeMenu(IWContext iwc) throws Exception {
		...
		if (this.showTypesAsDropdown) {
			...
			drp.setToSubmit();
			drp.setSelectedElement(this.currentType);
			Form F = new Form();
			F.add(drp);
			F.add(new HiddenInput(PRM_STAMP_P_FROM, String.valueOf(this.pFrom.getDate().getTime())));
			F.add(new HiddenInput(PRM_STAMP_P_TO, String.valueOf(this.pTo.getDate().getTime())));
			F.add(new HiddenInput(PRM_STAMP_U_FROM, String.valueOf(this.uFrom.getDate().getTime())));
			F.add(new HiddenInput(PRM_STAMP_U_TO, String.valueOf(this.uTo.getDate().getTime())));
			add(F);
		}
		else {
			Table T = new Table();
			...
			for (int i = 0; i < types.length; i++) {
				...
				typeLink.addParameter(PRM_STAMP_P_FROM, String.valueOf(this.pFrom.getDate().getTime()));
				typeLink.addParameter(PRM_STAMP_U_FROM, String.valueOf(this.uFrom.getDate().getTime()));
				typeLink.addParameter(PRM_STAMP_P_TO, String.valueOf(this.pTo.getDate().getTime()));
				typeLink.addParameter(PRM_STAMP_U_TO, String.valueOf(this.uTo.getDate().getTime()));
	...
	private PresentationObject getPrintedDatesForm() {
		...
		from.setYearRange(this.today.getYear() - 5, this.today.getYear() + 2);
		DateInput to = new DateInput(PRM_STAMP_P_TO, true);
		to = (DateInput) getStyledInterface(to);
		to.setYearRange(this.today.getYear() - 5, this.today.getYear() + 2);

		from.setDate(this.pFrom.getDate());
		to.setDate(this.pTo.getDate());
		...
		T.add(getCountDrop(PRM_P_COUNT, this.count_p), 4, 2);
		T.add(search, 6, 2);
	...
	private DropdownMenu getStatusDrop(String name, String selected) {
		DropdownMenu drp = new DropdownMenu(name);
		drp = (DropdownMenu) getStyledInterface(drp);
		drp.addMenuElement(this.statusUnprinted, localize("printdoc.unprinted", "Unprinted"));
		drp.addMenuElement(this.statusPrinted, localize("printdoc.printed", "Printed"));
		drp.addMenuElement(this.statusDeleted, localize("printdoc.deleted", "Deleted"));
	...
	private PresentationObject getUnPrintedDatesForm(boolean showStatus) {
		...
		to = (DateInput) getStyledInterface(to);
		to.setYearRange(this.today.getYear() - 5, this.today.getYear() + 2);
		from.setDate(this.uFrom.getDate());
		to.setDate(this.uTo.getDate());
		SubmitButton search = new SubmitButton(getResourceBundle().getLocalizedString("printdoc.fetch", "Fetch"));
		search = (SubmitButton) getButton(search);
		T.add(getHeader(getResourceBundle().getLocalizedString("printdoc.date_from", "From:")), 1, 1);
		T.add(from, 1, 2);
		...
		T.add(to, 2, 2);
		// T.add(getHeader(getResourceBundle().getLocalizedString("printdoc.count","Count")),5,1);
		T.add(getCountDrop(PRM_U_COUNT, this.count_u), 4, 2);
		if (showStatus) {
			T.add(getStatusDrop(PARAM_LETTER_STATUS, this.currentStatus), 5, 2);
		}
		T.add(search, 6, 2);
	...
	private PresentationObject getPrintButton() {
		...
		if (this.currentStatus.equals(this.statusDeleted)) {
	...
	private PresentationObject getRePrintButton() {
		...
		int col = 1;
		/*
		 * // make sure PRM_PROC_SELECTED parameter is set, for the form to be processed if (currentStatus.equals(statusDeleted)) { SubmitButton undelete =
		 * new SubmitButton(PRM_UNDEL_SELECTED,getResourceBundle().getLocalizedString("printdoc.undelete", "Undelete")); undelete = (SubmitButton)
		 * getButton(undelete); undelete.setSubmitConfirm( localize("printdoc.undelete_message", "Are you sure you want to mark these letters as unprinted
		 * ?")); T.add(new HiddenInput(PRM_PROC_SELECTED, "undel")); T.add(undelete, col++, 1); } else { SubmitButton delete = new
		 * SubmitButton(PRM_DEL_SELECTED,getResourceBundle().getLocalizedString("printdoc.delete", "Delete")); delete = (SubmitButton) getButton(delete);
		 * delete.setSubmitConfirm(localize("printdoc.delete_message", "Are you sure you want to mark these letters as deleted ?")); T.add(new
		 * HiddenInput(PRM_PROC_SELECTED, "del")); T.add(delete, col++, 1); }
		 *
		 * if (!currentStatus.equals(statusDeleted)) { CheckBox mark = getCheckBox("prv_mark",""); T.add(mark, col++, 1);
		 * T.add(getLocalizedHeader("printdoc.keep_unprinted", "Keep unprinted status"), col++, 1); } CheckBox bulk = new CheckBox("prv_bulk");
		 * T.add(bulk, col++, 1); T.add(getLocalizedHeader("printdoc.create_bulk_letter", "Bulk letter"), col++, 1);
		 */
		T.add(print, col++, 1);
		return T;
	}
	...
	private PresentationObject getCursorLinks(int totalsize, int cursor, String cursorPrm, int step) {
		...
		if (cursor > 0) {
			...
			prev.addParameter(PARAM_LETTER_TYPE, this.currentType);
			prev.addParameter(PARAM_LETTER_STATUS, this.currentStatus);
		...
		if (cursor <= (totalsize - step)) {
			...
			next.addParameter(PARAM_LETTER_TYPE, this.currentType);
			next.addParameter(PARAM_LETTER_STATUS, this.currentStatus);
	...
	private void addDateParametersToLink(Link link) {
		link.addParameter(PRM_STAMP_P_FROM, String.valueOf(this.pFrom.getDate().getTime()));
		link.addParameter(PRM_STAMP_U_FROM, String.valueOf(this.uFrom.getDate().getTime()));
		link.addParameter(PRM_STAMP_P_TO, String.valueOf(this.pTo.getDate().getTime()));
		link.addParameter(PRM_STAMP_U_TO, String.valueOf(this.uTo.getDate().getTime()));
		link.addParameter(PRM_U_COUNT, this.count_u);
		link.addParameter(PRM_P_COUNT, this.count_p);
	}
	...
	private void addDocumentsList(IWContext iwc) throws Exception {
		...
		// messageList.add("-");
		unPrintedLetterDocs.add(Integer.toString(getDocumentBusiness(iwc).getUnprintedMessagesCountByType(this.currentType)));

		Link printLink = new Link(localize("printdoc.print", "Print"));
		printLink.addParameter(PARAM_PRINT_UNPRINTED, "true");
		printLink.addParameter(PARAM_LETTER_TYPE, this.currentType);
		addDateParametersToLink(printLink);

		// if there are no letters to print nothing should happen, no error should be shown
		if (getDocumentBusiness(iwc).getUnprintedMessagesCountByType(this.currentType) == 0) {
		...
		viewUnprintedLink.addParameter(PARAM_LETTER_TYPE, this.currentType);
		...
		int count = this.cursor_p + 1;
		int ccp = this.count_p + this.cursor_p;
		Collection<PrintDocuments> printDocs = getDocumentBusiness(iwc).getPrintedDocuments(this.currentType, this.pFrom, this.pTo, this.count_p, this.cursor_p);
		...
		pT.add(getCursorLinks(printDocs.size(), this.cursor_p, PRM_CURSOR_P, this.count_p), 1, prow++);
		...
		Iterator<PrintDocuments> iter = printDocs.iterator();

		/*
		 * if (cursor_p > 0) { while (iter.hasNext() && cursor_p > 0) { iter.next(); cursor_p--; } }
		 */
		String view = localize("printdoc.view", "View");
		while (iter.hasNext() && count <= ccp) {
			PrintDocuments doc = iter.next();
	...
	/*
	 * Commented out since it is never used... private void addDocumentsListView(IWContext iwc) throws Exception { Form myForm = new Form(); Table T =
	 * new Table(); myForm.add(T); add(myForm);
	 *
	 * //int urow = 1;
	 *
	 * if (isBulkManual) T.add(new HiddenInput(PRM_BULK_VIEW, "true"));
	 *
	 * ColumnList printedLetterDocs = new ColumnList(4);
	 *
	 * Collection printDocs = getDocumentBusiness(iwc).getPrintedDocuments(currentType, pFrom, pTo);
	 *
	 * int prow = 1; Table hT = new Table(2, 1); hT.setWidth(hT.HUNDRED_PERCENT); hT.setAlignment(2, 1, hT.HORIZONTAL_ALIGN_RIGHT);
	 *
	 * hT.add(getLocalizedHeader("printdoc.printed_letters", "Printed letters"), 1, 1); Link singleMsgs = new Link(localize("printdoc.normal_view",
	 * "Normal view")); singleMsgs.addParameter(PARAM_LETTER_TYPE, currentType); singleMsgs.addParameter(PARAM_LETTER_STATUS, currentStatus);
	 * addDateParametersToLink(singleMsgs); hT.add(singleMsgs, 2, 1);
	 *
	 * T.add(hT, 1, prow++); T.add(getUnPrintedDatesForm(iwc, false), 1, prow++); T.add(printedLetterDocs, 1, prow++); T.add(getCursorLinks(iwc,
	 * printDocs.size(), cursor_p, PRM_CURSOR_P, count_p), 1, prow++);
	 *
	 * printedLetterDocs.setHeader("#", 1); printedLetterDocs.setHeader(localize("printdoc.printed_date", "Printing date"), 2);
	 * printedLetterDocs.setHeader(localize("printdoc.n_o_docs", "Number of documents"), 3); printedLetterDocs.setWidth(Table.HUNDRED_PERCENT);
	 *
	 * Iterator iter = printDocs.iterator(); int count = cursor_p + 1; int ccp = count_p + cursor_p; if (cursor_p > 0) { while (iter.hasNext() &&
	 * cursor_p > 0) { iter.next(); cursor_p--; } } while (iter.hasNext() && count <= ccp) { PrintDocuments doc = (PrintDocuments) iter.next();
	 * printedLetterDocs.add(String.valueOf(count)); printedLetterDocs.add(doc.getCreated().toString()); //messageList.add("-"); int subcount =
	 * doc.getNumberOfSubDocuments();
	 *
	 * Link namesLink = new Link(String.valueOf(subcount)); namesLink.addParameter(PARAM_VIEW_UNPRINTED, "true");
	 * namesLink.addParameter(PARAM_LETTER_TYPE, currentType); namesLink.addParameter(PARAM_LETTER_STATUS, currentStatus);
	 * namesLink.addParameter("prv_file_id", doc.getDocumentFileID()); addDateParametersToLink(namesLink); printedLetterDocs.add(namesLink);
	 *
	 * int fileID = doc.getDocumentFileID(); Link viewLink = new Link(localize("printdoc.view", "View")); viewLink.setFile(fileID);
	 * printedLetterDocs.add(viewLink); count++; } }
	 */

	/*
	 * Commented out since it is never used... private void addMessagesList(IWContext iwc) throws Exception { Form uForm = new Form(); Form pForm = new
	 * Form(); Table uT = new Table(); Table pT = new Table(); uForm.add(uT); pForm.add(pT); add(uForm); add(pForm); ColumnList unPrintedLetterDocs =
	 * new ColumnList(7);
	 *
	 * int urow = 1; Collection unprintedLetters = getMessageBusiness(iwc).getSingleUnPrintedLetterMessagesByType(currentType, uFrom, uTo);
	 * uT.add(getLocalizedHeader("printdoc.unprinted_letters", "Letters for printing"), 1, urow++); uT.add(getUnPrintedDatesForm(iwc, false), 1,
	 * urow++); uT.add(unPrintedLetterDocs, 1, urow++); uT.add(getPrintButton(), 1, urow++); uT.add( getCursorLinks(iwc, unprintedLetters.size(),
	 * cursor_u, PRM_CURSOR_U, count_u), 1, urow++);
	 *
	 * unPrintedLetterDocs.setWidth(Table.HUNDRED_PERCENT); unPrintedLetterDocs.setBackroundColor("#e0e0e0"); unPrintedLetterDocs.setHeader("#", 1);
	 * unPrintedLetterDocs.setHeader(localize("printdoc.created_date", "Message created"), 2);
	 * unPrintedLetterDocs.setHeader(localize("printdoc.receiver", "Receiver"), 3); unPrintedLetterDocs.setHeader(localize("printdoc.subject",
	 * "Subject"), 4);
	 *
	 * CheckBox checkAll = new CheckBox("checkall"); checkAll.setToCheckOnClick(PRM_U_CHK, true); unPrintedLetterDocs.setHeader(checkAll, 5);
	 * unPrintedLetterDocs.setHeader(localize("printdoc.file", "File"), 6); //unPrintedLetterDocs.setHeader(localize("printdoc.select","Select"),5);
	 * unPrintedLetterDocs.setHeader(localize("printdoc.bulk_file", "Bulk ID"), 7);
	 *
	 * Iterator iter = unprintedLetters.iterator(); int count = cursor_u + 1; int ccu = count_u + cursor_u; if (cursor_u > 0) { while (iter.hasNext() &&
	 * cursor_u > 0) { iter.next(); cursor_u--; } }
	 *
	 * int bulkId; while (iter.hasNext() && count <= ccu) { PrintedLetterMessage msg = (PrintedLetterMessage) iter.next();
	 * unPrintedLetterDocs.add(String.valueOf(count)); unPrintedLetterDocs.add(msg.getCreated().toString()); //messageList.add("-");
	 *
	 * unPrintedLetterDocs.add(msg.getOwner().getName());
	 *
	 * unPrintedLetterDocs.add(msg.getSubject()); if (useCheckBox) { CheckBox box = new CheckBox(PRM_U_CHK, msg.getPrimaryKey().toString());
	 * uForm.addParameter(PARAM_LETTER_TYPE, currentType); //uForm.addParameter(PARAM_LETTER_STATUS, currentStatus); unPrintedLetterDocs.add(box); }
	 * else { Link printLink = new Link(localize("printdoc.print", "Print")); printLink.addParameter(PARAM_PRINT_MSG, "true");
	 * printLink.addParameter(PARAM_LETTER_TYPE, currentType); printLink.addParameter(PARAM_LETTER_STATUS, currentStatus);
	 * printLink.addParameter(PARAM_MESSAGE_ID, msg.getPrimaryKey().toString()); addDateParametersToLink(printLink); unPrintedLetterDocs.add(printLink); }
	 * int fileID = msg.getMessageDataFileID(); if (fileID > 0) { Link viewLink = new Link(localize("printdoc.view", "View")); viewLink.setFile(fileID);
	 * unPrintedLetterDocs.add(viewLink); } else { unPrintedLetterDocs.add("-"); }
	 *
	 * bulkId = msg.getMessageBulkDataFileID(); if (bulkId > 0) { Link bulkLink = new Link(String.valueOf(msg.getMessageBulkDataFileID()));
	 * bulkLink.setFile(msg.getMessageBulkDataFileID()); unPrintedLetterDocs.add(bulkLink); } else { unPrintedLetterDocs.add("-"); } count++; }
	 *
	 * ColumnList printedLetterDocs = new ColumnList(6); printedLetterDocs.setWidth(Table.HUNDRED_PERCENT);
	 * printedLetterDocs.setBackroundColor("#e0e0e0"); printedLetterDocs.setHeader("#", 1);
	 * printedLetterDocs.setHeader(localize("printdoc.created_date", "Message created"), 2); printedLetterDocs.setHeader(localize("printdoc.receiver",
	 * "Receiver"), 3); printedLetterDocs.setHeader(localize("printdoc.subject", "Subject"), 4); printedLetterDocs.setHeader(localize("printdoc.file",
	 * "File"), 5); printedLetterDocs.setHeader(localize("printdoc.bulk_file", "Bulk File"), 6); Collection printedLetters =
	 * getMessageBusiness(iwc).getSinglePrintedLetterMessagesByType(currentType, pFrom, pTo);
	 *
	 * int prow = 1; pT.add(Text.getBreak(), 1, prow++); pT.add(getLocalizedHeader("printdoc.printed_letters", "Printed letters"), 1, prow++);
	 * pT.add(getPrintedDatesForm(iwc), 1, prow++); pT.add(printedLetterDocs, 1, prow++); pT.add( getCursorLinks(iwc, printedLetters.size(), cursor_p,
	 * PRM_CURSOR_P, count_p), 1, prow++);
	 *
	 * iter = printedLetters.iterator(); count = cursor_p + 1; int ccp = cursor_p + count_p; if (cursor_p > 0) { while (iter.hasNext() && cursor_p > 0) {
	 * iter.next(); cursor_p--; } } while (iter.hasNext() && count <= ccp) { PrintedLetterMessage msg = (PrintedLetterMessage) iter.next();
	 * printedLetterDocs.add(String.valueOf(count)); printedLetterDocs.add(msg.getCreated().toString()); //messageList.add("-");
	 * printedLetterDocs.add(msg.getOwner().getName()); printedLetterDocs.add(msg.getSubject()); int fileID = msg.getMessageDataFileID(); Link viewLink =
	 * new Link(localize("printdoc.view", "View")); viewLink.setFile(fileID); printedLetterDocs.add(viewLink); bulkId = msg.getMessageBulkDataFileID();
	 * if (bulkId > 0) {
	 *
	 * Link bulkLink = new Link(String.valueOf(bulkId)); bulkLink.setFile(bulkId); printedLetterDocs.add(bulkLink); } else printedLetterDocs.add("");
	 * count++; } }
	 */

	private PresentationObject getPrintedMessagesPart(IWContext iwc) throws RemoteException, FinderException {
		Table pT = new Table();
		...
		Collection printedLetters = getMessageBusiness(iwc).getSinglePrintedLetterMessagesByType(this.currentType, this.pFrom, this.pTo, this.count_p, this.cursor_p);
		...
		pT.add(getPrintedDatesForm(), 1, prow++);
		pT.add(printedLetterDocs, 1, prow++);
		pT.add(getCursorLinks(printedLetters.size(), this.cursor_p, PRM_CURSOR_P, this.count_p), 1, prow++);
		...
		int count = this.cursor_p + 1;
		/*
		 * int ccp = cursor_p + count_p; if (cursor_p > 0) { while (iter.hasNext() && cursor_p > 0) { iter.next(); cursor_p--; } }
		 */
		while (iter.hasNext()) {// && count <= ccp) {
			...
			/*
			 * bulkId = msg.getMessageBulkDataFileID(); if (bulkId > 0) {
			 *
			 * Link bulkLink = new Link(String.valueOf(bulkId)); bulkLink.setFile(bulkId); printedLetterDocs.add(bulkLink); } else
			 * printedLetterDocs.add("");
			 */
			count++;
		}
	...
	private PresentationObject getPrintedDocumentsPart(IWContext iwc) throws RemoteException, FinderException {
		Table T = new Table();
		T.setWidth(Table.HUNDRED_PERCENT);
		// int urow = 1;

		if (this.isBulkManual) {
			T.add(new HiddenInput(PRM_BULK_VIEW, "true"));
		}

		ColumnList printedLetterDocs = new ColumnList(4);

		Collection printDocs = getDocumentBusiness(iwc).getPrintedDocuments(this.currentType, this.pFrom, this.pTo, this.count_p, this.cursor_p);
		...
		T.add(getCursorLinks(printDocs.size(), this.cursor_p, PRM_CURSOR_P, this.count_p), 1, prow++);
		...
		while (iter.hasNext()) {// && count <= ccp) {
			...
			namesLink.addParameter(PARAM_LETTER_TYPE, this.currentType);
			namesLink.addParameter(PARAM_LETTER_STATUS, this.currentStatus);
	...
	private PresentationObject getViewLink() {
		...
		singleMsgs.addParameter(PARAM_LETTER_TYPE, this.currentType);
		singleMsgs.addParameter(PARAM_LETTER_STATUS, this.statusPrinted);
		addDateParametersToLink(singleMsgs);
		return singleMsgs;
	}

	private PresentationObject getUnprintedMessagesPart(IWContext iwc) throws RemoteException, FinderException {
		Table T = new Table();
		T.setWidth(Table.HUNDRED_PERCENT);
		ColumnList letterList = new ColumnList(7);

		int row = 1;
		Collection letters = getMessageBusiness(iwc).getSingleLettersByTypeAndStatus(this.currentType, this.currentStatus, this.uFrom, this.uTo, this.count_u, this.cursor_u);
		/*
		 * Table hT = new Table(2, 1); hT.setWidth(hT.HUNDRED_PERCENT); hT.setAlignment(2, 1, hT.HORIZONTAL_ALIGN_RIGHT);
		 *
		 * hT.add(getLocalizedHeader("printdoc.letters", "Letters"), 1, 1); Link singleMsgs = new Link(localize("printdoc.bulk_view", "Bulk view"));
		 * singleMsgs.addParameter(PRM_BULK_VIEW, "true"); singleMsgs.addParameter(PARAM_LETTER_TYPE, currentType);
		 * singleMsgs.addParameter(PARAM_LETTER_STATUS, currentStatus); addDateParametersToLink(singleMsgs); hT.add(singleMsgs, 2, 1);
		 */
		...
		T.setAlignment(1, row, Table.HORIZONTAL_ALIGN_RIGHT);
		T.add(getCursorLinks(letters.size(), this.cursor_u, PRM_CURSOR_U, this.count_u), 1, row++);
		...
		int count = this.cursor_u + 1;
		/*
		 * int ccu = count_u + cursor_u; if (cursor_u > 0) { while (iter.hasNext() && cursor_u > 0) { iter.next(); cursor_u--; } }
		 */
		// int bulkId;
		...
		while (iter.hasNext()) { // && count <= ccu) {
			...
			if (this.useCheckBox) {
				...
				printLink.addParameter(PARAM_LETTER_TYPE, this.currentType);
				printLink.addParameter(PARAM_LETTER_STATUS, this.currentStatus);
			...
			try {
				...
				if (this.userPrefPage != null) {
					Link noAddressLink = new Link(getErrorText("!"));
					noAddressLink.setPage(this.userPrefPage);
					noAddressLink.addParameter(this.UserIDPreferenceParameterName, msg.getOwner().getPrimaryKey().toString());
	...
	private void addUnPrintedNameList(IWContext iwc) throws Exception {
		...
		if (file_id > 0) {
			letters = getMessageBusiness(iwc).getLettersByBulkFile(file_id, this.currentType, this.currentStatus, -1, -1);
		}
		else {
			letters = getMessageBusiness(iwc).getUnPrintedLetterMessagesByType(this.currentType, -1, -1);
		}
		Iterator iter = letters.iterator();
		PrintMessage msg;
		User owner;
	...
	/*
	 * private boolean displayMessageToCommuneAdministrator(IWContext iwc, Message message) { boolean canDisplay = true; try {
	 *
	 * CaseCode caseCode = message.getParentCase().getCaseCode(); CommuneCaseBusiness ccBus = (CommuneCaseBusiness) IBOLookup.getServiceInstance(iwc,
	 * CommuneCaseBusiness.class); CaseCode[] codes = ccBus.getProviderCaseCodes(); for (int i = 0; i < codes.length && canDisplay; i++) { canDisplay =
	 * !codes[i].equals(caseCode); } } catch (Exception e) { e.printStackTrace(System.err); } return canDisplay; }
	 */
	...
	private boolean hideMessage(IWContext iwc, Message message) {
		...
		if (this.hiddenNonUserCaseCodes != null) {
			...
			for (Iterator iter = this.hiddenNonUserCaseCodes.iterator(); iter.hasNext();) {
	...
	/**
	 * Adds a hide filter to this viewer, the viewer then hides the letter if its parent case code equals the filter, unless the current user is the
	 * letter owner
	 *
	 * @param filter
	 */
	public void setHideCaseCodeFilter(String filter) {
		if (this.hiddenNonUserCaseCodes == null) {
			this.hiddenNonUserCaseCodes = new java.util.ArrayList();
		}
		this.hiddenNonUserCaseCodes.add(filter);
	}
